<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">New Saving</h2>
    </x-slot>

    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <h3 class="text-lg font-medium mb-4">New Saving</h3>
        ...
        <form action="{{ route('savings.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" value="{{ old('amount') }}" class="mt-1 block w-full border px-3 py-2" required>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ old('start_date') }}" class="mt-1 block w-full border px-3 py-2">
                    </div>
                    <div>
                        <label class="block text-sm font-medium">End date</label>
                        <input type="date" name="end_date" value="{{ old('end_date') }}" class="mt-1 block w-full border px-3 py-2">
                    </div>
                </div>

                <div class="flex items-center space-x-4">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="ongoing" class="mr-2"> Ongoing
                    </label>

                    <div class="w-full">
                        <label class="block text-sm font-medium">Interest rate (%)</label>
                        <input type="number" step="0.01" name="interest_rate" value="{{ old('interest_rate') }}" class="mt-1 block w-full border px-3 py-2">
                    </div>
                </div>
